...
/*
1. 通用模板和(偏)特化模板是平等的，互斥的，但是两者的优先级不一样，(偏)特化模板的优先级高于通用模板
2. 通用模板可继承(偏)特化模板(支持递归继承)，用于自动初始化非特化的模板。
3. (偏)特化模板的存在不依赖通用模板，两者相互独立，但是(偏)特化模板的参数必须满足通用模板的参数要求。
4. (偏)特化参数和通用模板参数(默认参数/指定参数)重合时，优先匹配偏特化模板, 因为偏特化优先级更高。
5. 参数匹配顺序: (1) 匹配通用模板检查并且确定参数，(2) 根据参数选择偏特化模板替换通用模板，(3) 是否替换成功?
6. ** (全)特化模板在编译期不能单独存在，全特化模板会被编译器视为具体类，需要有完整的实现 **。
*/
