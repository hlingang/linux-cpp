...
static void init_snapshot( struct snapshot_t* __snap )
{
    INIT_LIST_HEAD( &__snap->read_node );
    __snap->obj = nullptr;
}

static snapshot_t* create_snap()
{
    struct snapshot_t* new_obj = ( struct snapshot_t* )malloc( sizeof( struct snapshot_t ) );
    init_snapshot( new_obj );
    return new_obj;
}

static void destroy_snap( void* snap )
{
    if ( snap )
        free( snap );
}
...
void func_read( int id )
{
    ...
    obj->mtx.lock();
    cout << this_thread::get_id() << " Read Object ID: " << id << ", Value: " << obj->val << "[" << obj->s_count << "]"
         << endl;
...
restart:
    ...
    list_for_each_entry( obj, &obj_list, node )
    {
        ...
        list_mutex.unlock();
        obj->mtx.lock();
        cout << this_thread::get_id() << " ReadAll: Object ID: " << obj->id << ", Value: " << obj->val << "["
             << obj->s_count << "]" << endl;
        std::this_thread::sleep_for( std::chrono::milliseconds( 50 ) );
        obj->mtx.unlock();
        list_mutex.lock();
...
void func_read_all_snapshot()
{
    struct obj_data* obj;
    struct list_head snap_head;
    INIT_LIST_HEAD( &snap_head );
    list_mutex.lock();
    // 先制作快照 //
    list_for_each_entry( obj, &obj_list, node )
    {
        obj->s_count++;
        struct snapshot_t* snap = create_snap();
        snap->obj               = obj;
        list_add( &snap->read_node, &snap_head );
    }
    list_mutex.unlock();
    struct snapshot_t* snap;  // 遍历快照 //
    list_for_each_entry( snap, &snap_head, read_node )
    {
        obj = ( struct obj_data* )snap->obj;
        obj->mtx.lock();
        cout << this_thread::get_id() << " ReadAll: Object ID: " << obj->id << ", Value: " << obj->val << "["
             << obj->s_count << "]" << endl;
        obj->mtx.unlock();
    ...
    list_mutex.lock();
    while ( !list_empty( &snap_head ) )
    {
        struct snapshot_t* snap = list_entry( snap_head.next, struct snapshot_t, read_node );
        obj                     = ( struct obj_data* )snap->obj;
        list_del( &snap->read_node );
        __put_obj( obj );
        destroy_snap( snap );
    }
    list_mutex.unlock();
...
void func_write( int id )
{
    ...
    obj->mtx.lock();
    obj->val += "+";
    cout << this_thread::get_id() << " Write Object ID: " << id << ", Value: " << obj->val << "[" << obj->s_count << "]"
         << endl;
    std::this_thread::sleep_for( std::chrono::milliseconds( 50 ) );
    obj->mtx.unlock();
    put_obj( obj );
